...
public:
  ...
  std::pair<uint32_t, std::vector<uint8_t>> pack(uint32_t address, const std::vector<SignalPackValue> &values);
  std::pair<uint32_t, std::vector<uint8_t>> pack(const std::string &msg_name, const std::vector<SignalPackValue> &values);
