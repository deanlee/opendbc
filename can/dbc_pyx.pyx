...
cdef class DBCParser:
  ...
  def lookup_msg_id(self, msg_id):
    if not isinstance(msg_id, numbers.Number):
      msg_id = self.msg_name_to_address[msg_id.encode('utf-8')]
    return msg_id
  ...
  def decode22(self, x, arr=None, debug=False):
    ...
    for s in msg.sigs:
      ...
      start_bit = s.b1
      signal_size = s.b2
      little_endian = s.is_little_endian
      signed = s.is_signed
      factor = s.factor
      offset = s.offset
      ...
      if arr is None:
        ...
        out[arr.index(s.name)] = tmp
    return name, out

  def get_signals(self, msg):
    msg = self.lookup_msg_id(msg)
    return [sgs.name for sgs in self.address_to_msg[msg].sigs]
